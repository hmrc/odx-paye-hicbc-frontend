...
export default function TextInput(props) {
  const { name, errorText, hintText, inputProps = {}, maxLength, fieldId, onBlur, disabled } = props;
  ...
  const describedByIDs = `${hintText ? makeHintId(fieldId) : ''} ${errorText ? makeErrorId(fieldId) : ''}`.trim();
  ...
  return (
    <FormGroup {...props}>
      <input className={textInputClassesWithWidth(maxLength)} {...inputProps} id={fieldId} name={name} onBlur={onBlur}></input>
    </FormGroup>
  );
}
