...
export default function FieldSet({
  ...
  const formGroupDivClasses = `govuk-form-group ${errMessage || errorText ? 'govuk-form-group--error' : ''}`.trim();
  let legendClasses = ` ${legendIsHeading ? 'govuk-fieldset__legend govuk-fieldset__legend--l' : 'govuk-label govuk-label--m'}`.trim();

  // to updte legend style for Autocomplete
  legendClasses = isAutoCompleteField && !legendIsHeading ? 'govuk-label' : legendClasses;
  legendClasses = isAutoCompleteField && legendIsHeading ? 'govuk-fieldset__legend govuk-fieldset__legend--l' : legendClasses;
  ...
  const getErrMessagePCore = () => {
    if (typeof PCore !== 'undefined' && errMessage) {
      return PCore.getLocaleUtils().getLocaleValue(removeRedundantString(errMessage), 'Messages', PCore.getLocaleUtils().GENERIC_BUNDLE_KEY);
    }
    return errMessage;
  };
  ...
  return (
    <div className={formGroupDivClasses} {...testProps}>
      <fieldset className='govuk-fieldset' {...ariaDescBy} {...fieldsetElementProps}>
        ...
        {instructionText && legendIsHeading && <InstructionTextComponent instructionText={instructionText} />}
        {hintTextExists && (
