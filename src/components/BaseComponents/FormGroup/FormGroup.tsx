...
export default function FormGroup({
  ...
  useEffect(() => {
    ...
    setErrorMessage(found && errorMsgs.length <= 1 && found.message.fieldId === fieldId ? found.message.message : errorText);
  }, [errorText, errorMsgs]);
  ...
  return (
    <div className={formGroupDivClasses} {...testProps}>
      ...
      />
      {instructionText && labelIsHeading && <InstructionTextComponent instructionText={instructionText} />}
      {hintText && (
        <div id={makeHintId(fieldId)} className='govuk-hint'>
          <HintTextComponent htmlString={hintText} />
        </div>
      )}
      {(errMessage && errorEnabled) && (
