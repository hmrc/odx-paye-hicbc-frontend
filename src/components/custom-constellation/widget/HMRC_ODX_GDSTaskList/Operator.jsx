...
import { Link, Button, Popover, Flex, useOuterEvent, useElement, Progress, FormField, useTheme } from '@pega/cosmos-react-core';
...
const Operator = props => {
  ...
  const OperatorPreview = () => {
    ...
    setpopoverContent(<Progress variant='ring' message={localizedVal('Loading operator...', localeCategory)} placement='local' />);
    if (externalUser && externalUser.classID !== 'Data-Party-Operator') {
      const fields = [
        ...
        },
        {
          ...
          value: externalUser.email !== '' ? <Link href={`mailto:${externalUser.email}`}>{externalUser.email}</Link> : ''
        },
        {
          id: 'pyPhoneNumber',
      ...
      getOperatorDetails(id)
              ...
              },
              {
                ...
                value:
                  res.data.pyOperatorInfo.pyTelephone !== '' ? (
                    <Link href={`tel:${res.data.pyOperatorInfo.pyTelephone}`}>{res.data.pyOperatorInfo.pyTelephone}</Link>
                  ) : (
                    ''
                  )
              },
              {
                ...
                value:
                  res.data.pyOperatorInfo.pyEmailAddress !== '' ? (
                    <Link href={`mailto:${res.data.pyOperatorInfo.pyEmailAddress}`}>{res.data.pyOperatorInfo.pyEmailAddress}</Link>
                  ) : (
                    ''
                  )
              },
        ...
        .catch(() => {
          setIsLoading(false);
          setpopoverContent(<Flex container={{ pad: 1 }}>{localizedVal('Error loading the operator profile', localeCategory)}</Flex>);
        });
    }
  };
