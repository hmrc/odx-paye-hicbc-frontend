...
describe('should render component AppFooter', () => {
  ...
  it('should navigate to appropriate footer links when specific link is clicked.', () => {
    ...
    expect(screen.getByRole('link', { name: 'Cookies (opens in new tab)' })).toHaveAttribute('href', 'cookies');
    expect(screen.getByRole('link', { name: 'Accessibility statement (opens in new tab)' })).toHaveAttribute(
      'href',
      'accessibility'
    );
    ...
    expect(screen.getByRole('link', { name: 'Terms and conditions (opens in new tab)' })).toHaveAttribute(
      'href',
      'https://www.tax.service.gov.uk/help/terms-and-conditions'
    );
    expect(screen.getByRole('link', { name: 'Help using GOV.UK (opens in new tab)' })).toHaveAttribute('href', 'https://www.gov.uk/help');
    expect(screen.getByRole('link', { name: 'Contact (opens in new tab)' })).toHaveAttribute(
      'href',
      'https://www.gov.uk/government/organisations/hm-revenue-customs/contact'
    );
    expect(screen.getByRole('link', { name: 'Rhestr o Wasanaethau Cymraeg (opens in new tab)' })).toHaveAttribute(
      'href',
      'https://www.gov.uk/cymraeg'
    );
    expect(screen.getByRole('link', { name: 'Open Government Licence v3.0 (opens in new tab)' })).toHaveAttribute(
      'href',
      'https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/'
    );
    expect(screen.getByRole('link', { name: '© Crown copyright (opens in new tab)' })).toHaveAttribute(
      'href',
