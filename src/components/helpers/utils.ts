  ...
  if (list.length > 0) {
    ...
    if (newList.length > 0) {
      newList.forEach(element => {
        uniqueString = uniqueString + (uniqueString.length <= 0 ? '' : checkEmail ? '.' : '. ') + element.trim();
      });
    }
  }
  return uniqueString;
...
export const triggerLogout = (setIsLogout, isAutoSignout: boolean = false) => {
  ...
  const authServiceList = {
    gg: 'GovGateway',
    'gg-hicbc-dev': 'GovGateway-HICBC-Dev',
    'gg-hicbc': 'GovGateway-HICBC'
  };
  ...
  // const application = apiConfig['registration'].application;
  const application = isAutoSignout ? apiConfig[getJourneyName()]?.autoSignoutApplication : apiConfig[getJourneyName()]?.application;
  const regime = '';
...
export const getCurrentLanguage = () => sessionStorage.getItem('rsdk_locale')?.slice(0, 2).toLowerCase() || 'en';
