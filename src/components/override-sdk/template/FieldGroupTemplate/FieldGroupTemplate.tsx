...
import { ReadOnlyDefaultFormContext, GroupContext, ErrorMsgContext, ErrorMessage } from '../../../helpers/HMRCAppContext';
...
export default function Group(props) {
  const { children, heading, instructions, readOnly, getPConnect, helperText, placeholder, name } = props;
  ...
  // This is added to distinguish the extended date template from a normal field group as duplicate error is coming for non extended date field
  const extendedDateTemplateName = 'CaptureDate';
  const isExtendedDateTemplate = thePConn?.getMetadata()?.children?.[0]?.config?.name === extendedDateTemplateName;

  const { errorMsgs } = useContext(ErrorMsgContext);

  const childPConn = props.children[0]?.props.getPConnect();
  const pageReference: string = childPConn?.getPageReference(); // caseInfo.content
  const childName: string = childPConn?.getMetadata().config.context; // .POCDateOfBirth
  const childPageReference = `${pageReference}${childName}`; // caseInfo.content.POCDateOfBirth

  const errorMessages: ErrorMessage = errorMsgs.find(err => err.message?.pageReference === childPageReference)?.message || {};
  // {message: 'Your Date of Birth must include a day', pageReference: 'caseInfo.content.POCDateOfBirth', fieldId: 'POCDateOfBirth-Day', propertyName: '.Day'}
  ...
  if (children?.length > 0) {
    ...
    if (children[0].props?.getPConnect().getMetadata().type === 'Checkbox') {
      ...
      const getconfigAlternateDesignSystem = children[0].props.getPConnect().getMetadata().config.configAlternateDesignSystem;

      if (getconfigAlternateDesignSystem) {
        let stepId = getconfigAlternateDesignSystem?.stepId;
        stepId = stepId.split('@L ').pop();
        if (hasBeenWrapped && getconfigAlternateDesignSystem?.ShowChangeLink && inprogressStatus === 'Open-InProgress') {
          return (
            <GDSCheckAnswers
              label={heading}
              value={valuesList}
      ...
      children.forEach((child, index) => {
        ...
        const arrayExclusiveOptions = [t('I_DO_NOT_WORK_IN_ANY_OF_THESE_POSITIONS'), t('I_DO_NOT_HAVE_ANOTHER_REASON_TO_SEND_A_TAX_RETURN')];
    ...
    return (
      <GroupContext.Provider value={{ setErrorMessage }}>
        <FieldSet
          ...
          label={heading}
          legendIsHeading={isOnlyField}
          errorText={isExtendedDateTemplate ? message : errorMessage?.message?.message}
          {...props}
          name={childName}
        >
          {children}
