...
import { getServiceShutteredStatus, scrollToTop, removeRedundantString, getCurrentLanguage } from '../../../helpers/utils';
import { ErrorSummary } from 'hmrc-gds-react-components';
...
import { ErrorMsgContext, Errors } from '../../../helpers/HMRCAppContext';
...
export default function Assignment(props) {
  ...
  function checkErrorMessages() {
    ...
    /* if (containerName?.toLowerCase().includes('check your answer')) {
      errorStateProps = [
        {
          message: {
    ...
    for (const [key, value] of formFields) {
      ...
      } else if (type === 'Checkbox' || type === 'RadioButtons') {
        fieldId = `${formattedPageReference}-${formattedPropertyName}`;
      } else if (type === 'TextInput' && ['Day', 'Month', 'Year'].includes(fieldId)) {
        // Check for Extended Date Component TextInput only
        fieldId = `${formattedPageReference}-${formattedPropertyName}`;
      }

      const localizedMessage = localizedVal(validateMessage);

      // Remove duplicate error messages for extended date component
      const isDuplicate: boolean = errorStateProps.some(
        (err: Errors): boolean => err.message.message === localizedMessage && err.message.pageReference === pageReference
      );
  ...
  useEffect(() => {
    checkErrorMessages();
  }, [children, hasAutoCompleteError, t]);
        ...
        <div id='Assignment'>
          ...
          <MainWrapper>
            {errorMessages.length > 0 && (
              <ErrorSummary
                heading={t('THERE_IS_A_PROBLEM')}
                errorDetails={[
                  ...errorMessages.map(item => ({
                    message: localizedVal(item.message.message, localeCategory, localeReference),
                    targetFieldId: item.message.fieldId
                  }))
                ]}
              />
            )}
            ...
            <form noValidate>{renderAssignmentCard()}</form>
            <AskHMRC />
          </MainWrapper>
        </div>
      )}
