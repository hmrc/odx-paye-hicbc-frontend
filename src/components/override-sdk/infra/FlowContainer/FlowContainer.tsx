...
import { addContainerItem, showBanner, hasContainerItems } from '@pega/react-sdk-components/lib/components/infra/Containers/FlowContainer/helpers';
...
export const FlowContainer = props => {
  ...
  // Find the flow action ID
  const activeKey = useMemo(() => {
    const pCoreContainername = PCore.getContainerUtils().getActiveContainerItemName(`${PCore.getConstants().APP.APP}/primary`);
    const assignments = PCore.getStoreValue('.assignments', 'caseInfo', pCoreContainername);
    return assignments ? assignments[0]?.actions[0]?.ID : '';
  }, [props.getPConnectOfActiveContainerItem, props.getPConnect]);

  useLayoutEffect(() => {
    window.scrollTo(0, 0); // Scroll to top.
    
    if (!activeKey) return; // If we have no key, we can't accurately know if we're on a new screen.

    if (window.__lastScreenKey !== activeKey) {  // If we're on a new screen
      // Clear back button before rendering anything else. It will be recreated if it's meant to be on that screen.
      try {
        ...
        // Do nothing. If it doesn't exist, there's nothing to do.
      }

      // Track the last key
      window.__lastScreenKey = activeKey;
    }
  }, [activeKey]); // Refresh if activeKey changes.
  ...
  function hasAssignments() {
    ...
    const assignmentsList: any[] = ourPConn.getValue(pCoreConstants.CASE_INFO.D_CASE_ASSIGNMENTS_RESULTS, '');
