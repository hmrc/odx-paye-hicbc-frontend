...
export default function Date(props) {
  const { getPConnect, value = '', validatemessage, helperText, placeholder, readOnly, name, testId, configAlternateDesignSystem } = props;
  ...
  const [editedValidateMessage, setEditedValidateMessage] = useState(
    DateErrorFormatter(validatemessage, getPConnect().resolveConfigProps(getPConnect().getMetadata().config).label)
  );
  ...
  useEffect(() => {
    setEditedValidateMessage(
      // @ts-ignore
      localizedVal(DateErrorFormatter(validatemessage, getPConnect().resolveConfigProps(getPConnect().getMetadata().config).label))
    );
