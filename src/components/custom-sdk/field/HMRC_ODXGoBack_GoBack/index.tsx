...
export default function HmrcOdxGoBackGoBack(props: HmrcOdxGoBackGoBackProps) {
  const { getPConnect, caption, value, readOnly, testId, disabled, selectionList, referenceList } = props;
  ...
  function handleBackClick(isValueTrue: boolean) {
    if (isValueTrue) {
      ...
      if (!continueButton)
        {
           // For pages which doesn't have continue button call finishAssignment.
           actionsApi
             .finishAssignment(context)
             .then(() => {
               // eslint-disable-next-line no-console
               console.log('finishassignment');
             })
             .catch(() => {
 ...
 //     'CustomAssignmentFinishedInitiated'
  //  );
  //  return () => {
 //     PCore.getPubSubUtils().unsubscribe('CustomAssignmentFinishedInitiated', 'CustomAssignmentFinishedInitiated');
 //   };
 // }, []);
