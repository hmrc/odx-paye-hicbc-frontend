...
export default function HmrcOdxGdsSummaryCard(props) {
  ...
  switch (useType) {
    ...
    case '4':
      ...
      break;
    case '5':
      itemName = t('GDS_INFO_ITEM_PARTNERSHIP');
      break;
    default:
      break;
  }
  ...
  useEffect(() => {
    ...
    const values = formElms.map(field => {
      ...
      formattedValue =
        (field as any)?.props?.label === 'Date of birth' || (field as any)?.props?.label === 'Dyddiad geni' // TODO: Need to make more robust
  ...
  useEffect(() => {
    const updatedHiddenText = useType === 2 ? `${formattedValues[0]} ${childName}` : `${childName} ${formattedValues[0]}`;
    setHiddenText(updatedHiddenText);
  }, [childName, formattedValues]);

  const handleOnClick = (e, action: string) => {
    e.preventDefault();
    switch (action) {
      ...
      >
        <div className='govuk-summary-card'>
          <div className='govuk-summary-card__title-wrapper'>
            {itemName ? <h2 className='govuk-summary-card__title'>{itemName}</h2> : <div className='govuk-summary-card__title'>{itemName}</div>}
            <ul className='govuk-summary-card__actions'>
              {!isSingleEntity(pageReference, getPConnect) && (
                <li className='govuk-summary-card__action remove-link'>
                  {' '}
                  <a className='govuk-link' href='#' onClick={e => handleOnClick(e, t('GDS_ACTION_REMOVE'))}>
                    {t('GDS_ACTION_REMOVE')}
              ...
              <li className='govuk-summary-card__action'>
                {' '}
                <a className='govuk-link' href='#' onClick={e => handleOnClick(e, t('GDS_ACTION_CHANGE'))}>
                  {t('GDS_ACTION_CHANGE')}
                  <span className='govuk-visually-hidden'>{hiddenText}</span>
                </a>
              </li>
