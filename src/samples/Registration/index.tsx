...
import { initTimeout, staySignedIn } from '../../components/AppComponents/TimeoutPopup/timeOutUtils';
...
// import ShutteredServiceWrapper from '../../components/AppComponents/ShutterServicePage/ShutteredServiceWrapper';
...
const Registration: FunctionComponent<any> = ({ journeyName }) => {
    ...
    getSdkConfig().then(config => {
      PCore.getRestClient()
        ...
        .then(response => {
          ...
          const summaryData: any[] = response.data.data.caseInfo.content.ScreenContent.LocalisedContent;
  ...
  const serviceName = t('HICBC_HOMEPAGE_HEADING');
  ...
  function createCase() {
    ...
    PCore.getMashupApi()
      .createCase('HMRC-HICBC-Work-Registration', PCore.getConstants().APP.APP, {
        startingFields
      })
      .then(() => {
  ...
  function fetchInProgressRegistrationData() {
    ...
    PCore.getDataPageUtils()
      .getDataAsync('D_RegistrantWorkAssignmentHICBCCases', 'root', {
        CaseType: 'HMRC-HICBC-Work-Registration'
      })
      .then(resp => {
        if (!resp.resultCount) {
          createCase();
      ...
      () => {
        ...
        if (assignmentFinishedFlag !== 'true') {
          ...
          if (status === 'Resolved-Discarded') {
            ...
            PCore?.getPubSubUtils().unsubscribe(PCore.getConstants().PUB_SUB_EVENTS.CASE_EVENTS.END_OF_ASSIGNMENT_PROCESSING, 'assignmentFinished');
          } else {
            setShowPega(false);
            getResolutionContent();
          }
  ...
  // const isShutteredService = useServiceShuttered();
  const handleStartReg = () => {
    ...
    PCore.getMashupApi().createCase('HMRC-HICBC-Work-Registration', PCore.getConstants().APP.APP);
    setShowPega(true);
  };
  ...
  function startMashup() {
    ...
    PCore.onPCoreReady(async renderObj => {
      ...
      getSdkConfig()
        .then(sdkConfig => {
          if (sdkConfig.timeoutConfig.secondsTilLogout) millisecondsTillSignout = sdkConfig.timeoutConfig.secondsTilLogout * 1000;
        })
        .finally(() => {
          // Subscribe to any store change to reset timeout counter
          PCore.getStore().subscribe(() => {
            staySignedIn(setShowTimeoutModal, 'D_RegistrantWorkAssignmentHICBCCases', null, false, false, bShowResolutionScreen, {
              CaseType: 'HMRC-HICBC-Work-Registration'
            });
          });
      ...
      // fetchInProgressRegistrationData(); //original
    });
    ...
    PCore.getDataPageUtils()
      ...
      .then(resp => {
        ...
        if (isShuttered) {
          ...
          displayUserPortal();
          // BUG-12417 - to call create cases when shuttering is disabled
          handleStartReg();
          setShowPega(true);
        }
      })
    ...
    // Set the initial page content ready state to true
  ...
  useEffect(() => {
    document.addEventListener(
      ...
      () => {
        // start the portal
        startMashup();
        // BUG-12417 - to skip create cases when shuttering is enabled
        // handleStartReg();
        // setShowPega(true);
      },
      []
    ...
    return function cleanupSubscriptions() {
      ...
      PCore?.getPubSubUtils().unsubscribe(PCore.getConstants().PUB_SUB_EVENTS.ASSIGNMENT_OPENED, 'continueAssignment');
      PCore?.getPubSubUtils().unsubscribe(PCore.getConstants().PUB_SUB_EVENTS.CASE_OPENED, 'continueCase');
  ...
  const handleStaySignIn = (e?) => {
    ...
    staySignedIn(setShowTimeoutModal, 'D_RegistrantWorkAssignmentHICBCCases', null, false, true, bShowResolutionScreen, {
      CaseType: 'HMRC-HICBC-Work-Registration'
    });
  };
    ...
    if (serviceNotAvailable) {
      ...
      return (
        <>
          ...
          <LoadingWrapper pageIsLoading={!pageContentReady} spinnerProps={{ bottomText: t('LOADING'), size: '30px', label: t('LOADING') }}>
            {showUserPortal && (
  ...
  return (
    <>
      ...
      <AppHeader
        ...
        appname={t('HICBC_HOMEPAGE_HEADING')}
        hasLanguageToggle
        isPegaApp={bShowPega}
